...
const logger = createChildLogger('service-db');

export class ServiceDatabase {
  private db: sqlite3.Database | null = null;

  async initialize(): Promise<void> {
    const dbPath = config.database.serviceDbPath;
    ...
    return new Promise((resolve, reject) => {
      this.db = new sqlite3.Database(dbPath, async (err) => {
        ...
        try {
          ...
          logger.info('Service database initialized', { path: dbPath });
          resolve();
        } catch (error) {
          reject(error);
        }
    ...
    return new Promise((resolve, reject) => {
      this.db!.exec(schema, (err) => {
        if (err) {
          ...
          logger.info('Service database schema created');
          resolve();
        }
      });
    });
      ...
      if (this.db) {
        this.db.close((err) => {
          if (err) {
            ...
            logger.info('Service database closed');
            resolve();
          }
        });
      } else {
